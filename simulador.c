...
// Selecao do Mux3 --> EÂ´ soÂ´ colocar: 0, 1, 2 ... 7  para selecionar os Registradores

// Selecao do Mux4 --> EÂ´ soÂ´ colocar: 0, 1, 2 ... 7  para selecionar os Registradores ou 8 para entrar o nr. 1
...
#define INCHAR 53   // "110101"; -- INCHAR Rx     -- Rx[7..0] <- KeyPressed	Rx[15..8] <- 0Â´s  Format: < inst(6) | Rx(3) | xxxxxxx >
...
//processa uma linha completa e retorna o nÃºmero codificado
...
int main()
{
	...
	int LoadReg[8] = {0};
	int carry=0;// Flag do IR que indica se a ULA vai fazer operaÃ§Ã£o com carry ou nÃ£o 
	int opcode=0;
	int temp=0;
...
loop:
	...
	if(M1 > (TAMANHO_MEMORIA)) {
		M1 = 0;
		printf("  \n\nUltrapassou limite da memoria, coloque um jmp no fim do cÃ³digo\n");
		exit(1);
	}
	...
	// TODO talvez fazer isso depois da operaÃ§Ã£o da ula?
	temp = 0;
	for(i=16; i--; )        
...
int pega_pedaco(int ir, int a, int b) {
	...
	/* Essa operaÃ§Ã£o retira o numero do registrador entre o a-b bit
	   da instruÃ§Ã£o, realizando um right-shift de b posiÃ§Ãµes
	   e aplicando uma mÃ¡scara de n-bits, onde n = nr. 1's entre a e b
	   Obs.:    & - AND
	   >> - right-shift
